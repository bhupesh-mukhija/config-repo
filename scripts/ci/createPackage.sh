    ...
    then # if package name not found in sfdx project json (package not created yet)
        ...
        then # package with name not found
            ...
            then # if a version is already available for  commit id
                echo "A version current commit $LATEST_COMMIT already created, please remove the version and rerun the job"
                ...
                #VERSION_DEVHUB=$(echo $QUERY_RESPONSE | jq -r '"\(.result.records[0].MajorVersion)"+"."+"\(.result.records[0].MinorVersion)"+"."+"\(.result.records[0].PatchVersion)"')
                ...
                then
                    ...
                    if [ "$(isUpgrade ${MAIN_VERSION_DEVHUB//"."/ } ${VERSION_SFDX_JSON//"."/ })" = "1" ]
